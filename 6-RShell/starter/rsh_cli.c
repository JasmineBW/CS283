...
int exec_remote_cmd_loop(char *address, int port) {
    ...
    while (1) {
        ...
        if (cmd_buff[strlen(cmd_buff) - 1] != '\0') {
        ...
        int bytes_sent = send(cli_socket, cmd_buff, io_size, 0);
        if (bytes_sent == -1) {
            perror("header write error");
            return client_cleanup(cli_socket, cmd_buff, rsp_buff, ERR_RDSH_COMMUNICATION);
        }

        while ((bytes_ret = recv(cli_socket, rsp_buff, RDSH_COMM_BUFF_SZ, 0)) > 0) {
            if (bytes_ret < 0){
                perror("read error");
                return client_cleanup(cli_socket, cmd_buff, rsp_buff, ERR_RDSH_COMMUNICATION);
            }
            ...
            is_eof = ((char)rsp_buff[bytes_ret-1] == RDSH_EOF_CHAR) ? 1 : 0;
            if (is_eof) {
                rsp_buff[bytes_ret-1] = '\0'; 
            }
        
            printf("%.*s", (int)bytes_ret, rsp_buff);
