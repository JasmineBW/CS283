...
char *handle_error(int rc) {
    switch (rc) {
        case ERR_RDSH_COMMUNICATION:
            return CMD_ERR_RDSH_COMM;
        case ERR_RDSH_CMD_EXEC:
            return CMD_ERR_RDSH_EXEC;
        case ERR_RDSH_SERVER:
            return CMD_ERR_RDSH_ITRNL;
        case ERR_EXEC_CMD:
            return CMD_ERR_EXECUTE;
        default:
            return RCMD_SERVER_EXITED;
            break;
    }
}
...
int exec_client_requests(int cli_socket) {
    ...
    while(1) {
        ...
        if ((cmd_rc = rsh_built_in_cmd(&cmd_list.commands[0])) != BI_NOT_BI) {
            if (cmd_rc != OK) {
                if (cmd_rc == BI_CMD_EXIT || cmd_rc == BI_CMD_STOP_SVR) {
                    break;
                }
                else if (cmd_rc == BI_CMD_RC) {
                    sprintf(recv_buff, CMD_RC_MESSAGE, last_rc);
                }
                else if (cmd_rc == BI_CMD_DRAGON) {
                    if (print_dragon_to_socket(cli_socket) != OK) {
                        strcpy(recv_buff, handle_error(ERR_EXEC_CMD));
                    }   
                }
                last_rc = cmd_rc;
            } else {
            ...
            if (cmd_rc != OK) {
                strcpy(recv_buff, handle_error(cmd_rc));
                last_rc = cmd_rc;
             }
             strcpy(recv_buff, "");
             last_rc = cmd_rc;
