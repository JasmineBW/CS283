...
int get_student(int fd, int id, student_t *s){
    if (id == 0) {
        ...
        int sz = read(fd, s, STUDENT_RECORD_SIZE);
        if (sz == -1 || (sz != 0 && sz != STUDENT_RECORD_SIZE)) {
        printf(M_ERR_DB_READ);
        return ERR_DB_FILE;
        }
        
        if (sz == 0 || (sz == STUDENT_RECORD_SIZE && memcmp(s, &EMPTY_STUDENT_RECORD, STUDENT_RECORD_SIZE) == 0)) {
...
int add_student(int fd, int id, char *fname, char *lname, int gpa) {
    if (id < MIN_STD_ID || MAX_STD_ID < id) {
        printf(M_ERR_DB_READ);
        return ERR_DB_FILE;
    }
    ...
    student_t *s = malloc(STUDENT_RECORD_SIZE);
    if (s == NULL) {
        printf(M_ERR_DB_READ);
        return ERR_DB_FILE;
    }
    ...
    if (rc == ERR_DB_FILE) {
        ...
        int write_sz = write(fd, &student, STUDENT_RECORD_SIZE);
        if (write_sz == -1) {
...
int del_student(int fd, int id){
    ...
    if (get_student(fd, id, s) == NO_ERROR) {
        ...
        int write_sz = write(fd, &EMPTY_STUDENT_RECORD, STUDENT_RECORD_SIZE);
        if (write_sz == -1) {
            printf(M_ERR_DB_WRITE);
            return ERR_DB_FILE;
        }
...
int count_db_records(int fd){
    ...
    while ((sz = read(fd, s, STUDENT_RECORD_SIZE)) == STUDENT_RECORD_SIZE) {
